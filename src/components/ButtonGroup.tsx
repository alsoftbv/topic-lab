            ...
            <button className={`button-group-header ${isGroupSelected ? 'group-selected' : ''}`} onClick={handleHeaderClick} title={`Toggle (${mod}T)`}>
                ...
                <span className="button-group-actions" onClick={(e) => e.stopPropagation()}>
                    ...
                    {!isUngrouped && (
                        <>
                            {isRenaming ? (
                                <button className="btn-icon" onMouseDown={(e) => { e.preventDefault(); handleSaveRename(); }} title="Save">
                                    <Check size={14} />
                                </button>
                            ) : (
                ...
                {!isUngrouped && (
                    <span
                        ...
                        onClick={(e) => e.stopPropagation()}
                        onMouseDown={(e) => { e.preventDefault(); e.stopPropagation(); group && onGroupDragStart(group.id); }}
